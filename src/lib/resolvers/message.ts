...
import { Message, PermissionsBitField, Snowflake, User } from 'discord.js';
...
async function getMessageFromChannel(channelId: Snowflake, messageId: Snowflake, originalAuthor: User): Promise<Message | null> {
	...
	if (!channel.permissionsFor(originalAuthor)?.has(PermissionsBitField.Flags.ViewChannel)) return null;
